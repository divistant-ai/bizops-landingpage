...
const EmptyState: React.FC<EmptyStateProps> = memo(
  ({ type = 'no-data', icon, title, description, actionLabel, onAction, className = '' }) => {
    const Icon = icon || defaultIcons[type];
    const iconColor
      = type === 'error' ? 'text-red-500 dark:text-red-400' : 'text-slate-400 dark:text-slate-600';
    ...
    return (
      ...
      >
        <div className={cn('h-16 w-16', iconColor, 'mb-4 flex items-center justify-center')}>
