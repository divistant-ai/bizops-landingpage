...
export default function RoadmapPage() {
  ...
  return (
    <>
        ...
        <div className="relative z-10">
          <Container size="7xl" className="mb-12">
            <Stack
              direction="vertical"
              gap={6}
              className="items-end justify-between border-b border-slate-200 pb-8 dark:border-slate-800"
            >
              <div className="max-w-2xl">
                ...
                <Typography variant="body" className="text-slate-600 dark:text-slate-400">
                  Transparansi adalah budaya kami. Lihat fitur apa saja yang sedang kami kembangkan,
                  vote untuk prioritas Anda, atau ajukan ide fitur baru.
                </Typography>
              </div>
              <Button
                onClick={() => setShowRequestForm(true)}
                className="bg-primary-600 hover:bg-primary-700 text-slate-800 dark:text-white"
              >
                <Lightbulb className="mr-2 h-4 w-4 text-slate-800 dark:text-white" />
                Request Feature
              </Button>
            </Stack>
                ...
                {allTags.map(tag => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                      activeFilter === tag
                        ? 'border border-slate-500 bg-slate-100 text-slate-900 dark:border-slate-50 dark:bg-slate-800 dark:text-white'
          ...
          <Container size="7xl" className="space-y-12">
            {roadmapData.map((column, colIdx) => {
              ...
              return (
                <div
                  key={colIdx}
                  className={`border-t-4 ${getStatusColor(column.color)} rounded-2xl bg-white p-8 shadow-lg dark:bg-slate-900`}
                >
                  ...
                  <Grid cols={1} gap={4}>
                    {filteredItems.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="hover:border-primary-500 dark:hover:border-primary-500 rounded-xl border border-slate-200 p-6 transition-colors dark:border-slate-800"
                        >
          ...
          <Container size="6xl" className="mt-16">
            <div className="from-primary-900 to-primary-800 rounded-3xl bg-linear-to-br p-12 text-center text-white">
              ...
              <p className="mx-auto mb-8 max-w-2xl text-lg text-slate-800 dark:text-white">
                Roadmap ini terus berkembang berdasarkan feedback Anda. Jangan ragu untuk mengajukan
                ide fitur baru!
              </p>
              <Button
                onClick={() => setShowRequestForm(true)}
              ...
              >
                <MessageSquare className="mr-2 h-4 w-4 text-slate-800 dark:text-white" />
                Share Your Ideas
              </Button>
            </div>
          </Container>
      ...
      {/* Feature Request Modal - Outside main content */}
      <AnimatePresence>
        {showRequestForm && (
          <motion.div
            initial={{ opacity: 0 }}
